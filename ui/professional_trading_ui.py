# ui/professional_trading_ui.py - COMPLETE FIXED VERSION
...
class ProfessionalTradingUI:
    def __init__(self, root, analyzer, provider, paper_trader, *, paper_mode=True, **kwargs):
        ...
        self.settings_password = "1234"  # Default password for settings

        self.version = kwargs.get("version") or "3.0.0"
        self.build = kwargs.get("build") or ""
        self.app_title = kwargs.get("title") or "Angel One Trading Bot"
        self.starting_tab = kwargs.get("starting_tab") or "Dashboard"

        # Trading parameters
        self.max_positions = 5
        self.capital_per_position = 20000  # Base capital per stock
    ...
    def _run_premarket_analysis(self):
        try:
            result = self.analyzer.run_premarket_analysis(self.watchlist)
...
Analyzed: {result['total_symbols_analyzed']} | Signals: {result['signals_found']} | High Confidence: {result['high_confidence_signals']}"""
            ...
            messagebox.showerror("Error", f"Pre-market analysis failed: {str(e)}")
    ...
    def _build_analyzer_tab(self, parent):
        ...
        # Add analysis info panel like Pre-Market
...
Analyzed: {len(self.watchlist)} | Signals Found: {len(signals)} | High Confidence: {high_conf}"""
    ...
    def _on_position_click(self, event):
        """Handle clicks on position tree - check if Exit button clicked"""
    ...
    def _refresh_paper_tab(self):
        """Refresh paper trading positions with Exit buttons"""
        ...
        for sym, pos in holdings.get('positions', {}).items():
            ...
            if 'sl' in pos and pos['sl'] > 0 and current_price <= pos['sl']:
                self.paper_trader.sell(sym, pos['qty'], current_price)
            elif 'target' in pos and pos['target'] > 0 and current_price >= pos['target']:
                self.paper_trader.sell(sym, pos['qty'], current_price)
    ...
    def _build_about_tab(self, parent):
        ...
        about_text = f"""ANGEL ONE TRADING BOT - MODULAR ARCHITECTURE
...
• Sentiment Analysis (requires NewsAPI key)
...
Warning: This is educational software. Always test thoroughly in paper mode before live trading."""
    ...
    def _select_starting_tab(self):
        tabs = {"Dashboard": 0, "Pre-Market": 1, "Watchlist": 2, "Analyzer": 3, "Paper Trade": 4, "Logs": 5, "About": 6}
    ...
    def _refresh_dashboard(self):
        """
        FIXED: Refresh dashboard - ALWAYS fetch real broker data when connected
        Shows both paper trading AND real broker holdings/funds
        """
        # Provider snapshot
        try:
            snap = self.provider.snapshot()
            self._provider_text.delete("1.0", "end")
            self._provider_text.insert("end", json.dumps(snap, indent=2))
        except Exception as e:
            self._provider_text.delete("1.0", "end")
            self._provider_text.insert("end", f"Error: {e}")

        # Holdings - ALWAYS fetch real broker holdings when connected
        try:
            if self.provider.is_connected:
                # Get REAL broker holdings
                broker_holdings = self.provider.get_holdings()
                
                if self.paper_mode:
                    # Show BOTH paper and real holdings
                    ...
                    # Live mode - show only broker holdings
                    self._holdings_text.delete("1.0", "end")
                    self._holdings_text.insert("end", json.dumps(broker_holdings, indent=2))
            else:
                # Not connected - show paper holdings only
                paper_holdings = self.paper_trader.holdings_snapshot()
                self._holdings_text.delete("1.0", "end")
                self._holdings_text.insert("end", json.dumps(paper_holdings, indent=2))
                
        except Exception as e:
            self._holdings_text.delete("1.0", "end")
            self._holdings_text.insert("end", f"Error: {e}")

        # Margin/Funds - ALWAYS fetch real broker funds when connected
        try:
            self._account_name.set(self.provider.account_name())
            
            if self.provider.is_connected:
                # Get REAL broker funds
                ...
                # Not connected - show paper only
                paper_margin = self.paper_trader.get_available_margin()
                self._netcash_text.set(f"₹{paper_margin:,.2f}")
