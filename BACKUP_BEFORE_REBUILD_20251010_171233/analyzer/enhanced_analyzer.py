...
class EnhancedAnalyzer:
    ...
    def analyze_symbol(self, symbol, exchange="NSE"):
        ...
        try:
            ...
            time.sleep(0.5)  # 500ms delay between historical requests
    ...
    def analyze_watchlist(self, symbols, exchange="NSE"):
        ...
        # ✅ FIXED: Added rate limiting and progress tracking
        import time
        ...
        for idx, sym in enumerate(symbols):
            ...
            if idx > 0 and idx % 10 == 0:
                print(f"  ⏳ Progress: {idx}/{total_symbols} stocks analyzed... ({len(signals)} signals so far)")
            ...
            # ✅ CRITICAL: Add delay to prevent rate limiting
            # Wait 250ms between stocks = max 4 requests per second
            if idx < total_symbols - 1:  # Don't wait after last stock
                time.sleep(0.25)
            ...
            if idx < total_symbols - 1:
                time.sleep(0.25)  # 250ms = max 4 stocks/sec
        signals.sort(key=lambda x: x['confidence'], reverse=True)
        logger.info(f"Watchlist analysis complete. Found {len(signals)} valid signals.")
        # ✅ Return top 10 highest confidence signals only
        top_signals = signals[:10]
        print(f"✅ Analysis complete: Returning {len(top_signals)} of {len(signals)} signals (top 10)")
        return top_signals
